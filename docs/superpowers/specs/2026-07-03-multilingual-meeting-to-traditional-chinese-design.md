...
5. **（2026-07-03 修訂）泰文原樣保留、不翻譯**：因 Whisper 對混語音檔常誤辨泰語，硬翻只會產生「篤定但錯」的中文；改為**中文→繁體、英文→繁體、泰文保留原始泰文**。產物為繁體中文（來自中／英）＋ 泰文原文（供人工判讀）。注意此規則只在 ASR 確實把該段轉成泰文字時有效；若泰語音訊被誤辨成他語 token，翻譯層無從辨識、無法救回。
...
- 指示 LLM 逐行判斷中／泰／英：中文保留原意僅確保繁體；**泰文原樣保留、不翻譯**；英文翻成自然繁體；混語行的中／英部分轉繁、泰文部分保留。
...
## 風險／限制（誠實揭露）
- **泰文 ASR 品質是最大天花板**：Whisper large + auto 對碼切換音檔常誤斷句/誤辨泰文。**（修訂後）**泰文改為原樣保留、不翻譯 —— 這避免了「拿誤辨泰文硬翻成篤定錯中文」，但**前提是 ASR 至少把該段轉成泰文字**；若泰語音訊被誤辨成他語 token，LLM 看到的就不是泰文、無從保留，翻譯層救不回。
- **泰文原文需人工後製**：產物含未翻譯的泰文行，交付前通常仍需人工判讀/補譯。
...
```
# Role: 資深多語會議字幕翻譯專家
您精通繁體中文（台灣）、泰語與英語。輸入字幕來自一場混合語言的會議，每一行可能是繁體中文、泰語或英語且交錯出現。您的任務是把「中文與英文」輸出為繁體中文（台灣），並「原樣保留泰語」。

# 逐行語言處理規則：
1. 該行已是中文：保持原意，僅確保為繁體中文（台灣用字用詞），不改寫語氣、不增刪內容。
2. 該行是泰語：原樣保留該行泰文，不翻譯、不改寫（混語音辨識泰語常有誤，寧留原文供人工判讀）。
3. 該行是英語：翻譯為自然流暢的繁體中文（台灣）。
4. 同行混用多語：中文與英文部分轉為繁體中文，泰文部分原樣保留。
...
# 輸出格式（極重要）：
1. 嚴格按輸入 JSON 格式輸出，保留原始鍵（ID），只依規則處理值。
...
Output:
{"0": "各位早安，今天開會", "1": "สวัสดีครับ ยินดีที่ได้พบทุกคน", "2": "我們先從預算開始"}
```
