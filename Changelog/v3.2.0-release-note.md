...
- 新增「云端听写」引擎：接入 OpenAI 兼容的在线 ASR（如 whisper-1 / gpt-4o-transcribe）、ElevenLabs Scribe、Deepgram、火山引擎豆包（录音文件识别·极速版）、腾讯云（录音文件识别极速版，每月赠 5 小时免费额度）与阿里云（录音文件识别极速版，识别语种在控制台项目中配置；仅商用版），免 GPU、可配置多服务商多实例并逐任务选择；优先复用词级时间戳走内置成句管线，缺时间戳时按静音切片降级，大文件自动压缩/切片。音频会上传到你配置的第三方端点，首次运行有隐私确认
  Added a "Cloud ASR" engine: OpenAI-compatible online transcription (e.g. whisper-1 / gpt-4o-transcribe), ElevenLabs Scribe, Deepgram, Volcengine Doubao (flash file recognition), Tencent Cloud (flash file recognition, 5 free hours per month) and Alibaba Cloud (flash file recognition; language configured on the console project; commercial-only), no GPU needed, with multi-provider/multi-instance config selectable per task; reuses word-level timestamps through the built-in segmentation pipeline, degrades via silence chunking when timestamps are missing, and auto-compresses/splits large files. Audio is uploaded to the endpoint you configure, with a first-run privacy confirmation
